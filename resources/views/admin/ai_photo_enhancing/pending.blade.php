...
@section('title', 'AI Photo Enhancing - Pending Products')
...
@section('content')
    ...
    <div
        ...
        id="newSpecificationSection" style="display:block;">
...
<div class="ai-content-card mt-4">
    ...
    <div
        ...
        id="allSpecificationsSection" style="display:block;"
        style="display:block;">
        ...
        <div class="section-header">
            ...
            <div class="list-header-actions">
                ...
                <div class="search-box me-2">
                    <select id="supplierFilter" class="form-select select2-master" data-placeholder="All Suppliers">
                ...
                <tbody id="specificationEmpty" style="display:none;">
                    <tr>
                        <td colspan="7" class="text-center py-5">
                            ...
                            <h6 class="mt-3">No design specifications found</h6>
                            <p class="text-muted mb-0">Create your first garment design specification.</p>
                        </td>
                    </tr>
                </tbody>
...
{{-- =============================================================
     ASSIGN ENHANCERS MODAL
============================================================= --}}
<div class="modal fade" id="assignEnhancersModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content border-0 shadow-lg">
            <div class="modal-header bg-light border-bottom-0">
                <h5 class="modal-title fw-bold text-dark">
                    <i class="bi bi-person-plus text-primary me-2"></i> Assign AI Photo Enhancers
                </h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
            </div>
            <div class="modal-body p-4">
                <form id="assignEnhancersForm">
                    <input type="hidden" id="assign_specification_id" name="specification_id" value="">
                    
                    <div class="mb-4">
                        <label class="form-label fw-semibold text-secondary mb-2">Select Enhancers</label>
                        <select id="assign_enhancers_select" name="enhancer_ids[]" class="form-select" multiple="multiple" data-placeholder="Choose enhancers..." required>
                            @foreach($aiPhotoEnhancers as $enhancer)
                                <option value="{{ $enhancer->sno }}">{{ $enhancer->first_name }} {{ $enhancer->last_name }}</option>
                            @endforeach
                        </select>
                        <div class="form-text mt-2"><i class="bi bi-info-circle me-1"></i> You can select multiple enhancers for this product.</div>
                    </div>
                </form>
            </div>
            <div class="modal-footer bg-light border-top-0">
                <button type="button" class="btn btn-outline-secondary px-4 rounded-pill" data-bs-dismiss="modal">Cancel</button>
                <button type="button" class="btn btn-primary px-4 rounded-pill shadow-sm" id="btnSubmitAssign">
                ...
                if (btnRefresh) {
                    ...
                    btnRefresh.addEventListener(
                        ...
                        function() {
                            ...
                            if (sf) {
                                $(sf).val('all').trigger('change.select2'); // Use change.select2 to update UI but avoid infinite loop if on('change') exists
                            }
            ...
            */
            ...
            showNewSpecification();
                ...
                function loadSpecifications(
                    ...
                    fetch(
                        "{{ route('admin.ai-photo-enhancing.pending.data') }}?" +
                        params.toString(),
                        {
                            method: 'GET',
                ...
                ====================================================== */
                    ...
                    tr.innerHTML = `
                        ...
                        <td class="text-end">
                            <button
                                type="button"
                                class="btn btn-sm btn-outline-success btn-assign-spec mb-1"
                                data-id="${escapeHtml(specId)}">
                                <i class="bi bi-send"></i> Send to Photo
                            </button>
                            <button
                                type="button"
                                class="btn btn-sm btn-outline-primary btn-view-spec mb-1"
                                data-id="${escapeHtml(specId)}">
                    ...
                    if (viewBtn) {
                        viewBtn.addEventListener('click', function() {
                            if (typeof openSpecificationView === 'function') { 
                                ...
                                console.error('openSpecificationView is not defined as a function!');
                            }
                        });
                    }

                    const assignBtn = tr.querySelector('.btn-assign-spec');
                    if (assignBtn) {
                        assignBtn.addEventListener('click', function() {
                            openAssignModal(specId);
                        });
                    }
...
function openLargeImage(
                        ...
                        ====================================================== */
                        /* =====================================================
                        ASSIGN ENHANCERS LOGIC
                        ====================================================== */
                        let assignModalInstance = null;
                        
                        function openAssignModal(specId) {
                            document.getElementById('assign_specification_id').value = specId;
                            
                            // Initialize select2 if not already initialized
                            if (!$('#assign_enhancers_select').hasClass('select2-hidden-accessible')) {
                                $('#assign_enhancers_select').select2({
                                    dropdownParent: $('#assignEnhancersModal'),
                            ...
                            $('#assign_enhancers_select').val(null).trigger('change');
                            
                            const modalEl = document.getElementById('assignEnhancersModal');
                        ...
                        // Expose function globally so it can be called from dynamically generated buttons
                        window.openAssignModal = openAssignModal;
                        
                        document.getElementById('btnSubmitAssign').addEventListener('click', function() {
                            const specId = document.getElementById('assign_specification_id').value;
                            const enhancerIds = $('#assign_enhancers_select').val();
                            
                            if (!enhancerIds || enhancerIds.length === 0) {
                                alert('Please select at least one AI Photo Enhancer.');
                                return;
                            }
                            ...
                            fetch("{{ route('admin.ai-photo-enhancing.assign') }}", {
                                method: 'POST',
                                headers: {
                            ...
                            .then(data => {
                                ...
                                if (data.success) {
                                    ...
                                    const btnRefresh = document.getElementById('btnRefreshSpecifications');
                                    if (btnRefresh) {
                                        btnRefresh.click();
                                    } else {
                                    ...
                                    alert(data.message || 'An error occurred while assigning the product.');
                                }
                            })
                            .catch(error => {
                                this.disabled = false;
